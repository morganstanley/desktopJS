...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  container.ready().then(() => {
    ...
    $("#button-joingroup").prop(
      "disabled",
      !container.getCurrentWindow().allowGrouping
    );
    $("#button-leavegroup").prop(
      "disabled",
      !container.getCurrentWindow().allowGrouping
    );

    container.addListener("window-created", (e) =>
      container.log(
        "info",
        "Window created: " + e.window + ", " + e.windowId + ", " + e.windowName
      )
    );
    container.addListener("layout-loaded", (e) =>
      container.log("info", "Layout loaded")
    );
    container.addListener("layout-saved", (e) =>
      container.log("info", "Layout saved")
    );

    desktopJS.Container.addListener("window-created", (e) =>
      container.log(
        "info",
        "Window created - static (Container): " +
          e.windowId +
          ", " +
          e.windowName
      )
    );
    desktopJS.ContainerWindow.addListener("window-created", (e) =>
      container.log(
        "info",
        "Window created - static (ContainerWindow): " +
          e.windowId +
          ", " +
          e.windowName
      )
    );
    desktopJS.Container.addListener("layout-saved", (e) =>
      container.log("info", "Layout saved - static: " + e.layoutName)
    );
    desktopJS.Container.addListener("layout-loaded", (e) =>
      container.log("info", "Layout loaded - static: " + e.layoutName)
    );

    desktopJS.ContainerWindow.addListener("window-joinGroup", (e) =>
      container.log("info", "grouped " + JSON.stringify(e))
    );
    desktopJS.ContainerWindow.addListener("window-leaveGroup", (e) =>
      container.log("info", "ungrouped" + JSON.stringify(e))
    );
    ...
    if (container.getCurrentWindow().id === "desktopJS") {
      snapAssist = new desktopJS.SnapAssistWindowManager(container, {
        windowStateTracking:
          desktopJS.WindowStateTracking.Main |
          desktopJS.WindowStateTracking.Group,
      });
    }
  });
});
...
trayIconButton.onclick = function () {
  container.addTrayIcon(
    { icon: "assets/img/application.png", text: "ContainerPOC" },
    function () {
      container
        .getMainWindow()
        .isShowing()
    ...
    [
      {
        ...
        click: function (menuItem) {
          window.alert(menuItem.label + " clicked");
        },
      },
      {
        label: "Test 1",
        icon: "assets/img/application.png",
        click: function (menuItem) {
          window.alert(menuItem.label + " clicked");
        },
      },
...
childFocusButton.onclick = function () {
  childWindow.show().then(childWindow.focus());
};
...
function subscribe() {
  ...
  container.ipc
    .subscribe("stock.selected", function (event, message) {

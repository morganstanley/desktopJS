...
function createWindow() {
  ...
  snapAssist = new desktopJS.SnapAssistWindowManager(container, {
    windowStateTracking:
      desktopJS.WindowStateTracking.Main | desktopJS.WindowStateTracking.Group,
  });

  container
    .createWindow("http://localhost:8000", { name: "desktopJS", main: true })
    .then((win) => (mainWindow = win));

  let trayIcon = electron.nativeImage.createFromPath(
    __dirname + "\\..\\web\\favicon.ico"
  );
  container.addTrayIcon(
    { icon: trayIcon, text: "ContainerPOC" },
    () => {

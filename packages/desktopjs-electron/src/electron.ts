  ...
  condition: () => {
    try {
      return (
        ...
        (require("electron") || require("electron").remote)
      );
    } catch (e) {
      return false;
    }
...
export class ElectronContainerWindow extends ContainerWindow {
  ...
  public getGroup(): Promise<any[]> {
    return new Promise<ContainerWindow[]>((resolve) => {
      const ids = this.isRemote
        ? (<any>(
            this.container.internalIpc
          )).sendSync(InternalMessageType.getGroup, { source: this.id })
      ...
      resolve(
        ids.map((id) => {
          return id === this.id
            ? this
            : this.container.wrapWindow(
                this.container.browserWindow.fromId(id)
              );
        })
      );
    });
  ...
  public joinGroup(target: ContainerWindow): Promise<void> {
    ...
    if (this.isRemote) {
      (<any>this.container.internalIpc).send(InternalMessageType.joinGroup, {
        source: this.id,
        target: target.id,
      });
    } else {
      (<any>this.container).windowManager.groupWindows(
        target.innerWindow,
        this.innerWindow
      );
    }
  ...
  public leaveGroup(): Promise<void> {
    if (this.isRemote) {
      (<any>this.container.internalIpc).send(InternalMessageType.leaveGroup, {
        source: this.id,
      });
    } else {
  ...
  public getOptions(): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      const options = this.isRemote
        ? (<any>(
            this.container.internalIpc
          )).sendSync(InternalMessageType.getOptions, { source: this.id })
  ...
  public getState(): Promise<any> {
    if (this.innerWindow && this.innerWindow.webContents) {
      return this.innerWindow.webContents.executeJavaScript(
        "window.getState ? window.getState() : undefined"
      );
    } else {
  ...
  public setState(state: any): Promise<void> {
    const promise =
      this.innerWindow && this.innerWindow.webContents
        ? this.innerWindow.webContents.executeJavaScript(
            `if (window.setState) { window.setState(JSON.parse(\`${JSON.stringify(
              state
            )}\`)); }`
          )
        : Promise.resolve();
  ...
  protected attachListener(
    ...
    if (eventName === "beforeunload") {
      ...
      if (win && this.id === this.container.getCurrentWindow().id) {
        ...
        throw new Error(
          "Event handler for 'beforeunload' can only be added on current window"
        );
      }
    } else {
      this.innerWindow.addListener(
  ...
  protected detachListener(
    ...
    this.innerWindow.removeListener(
      windowEventMap[eventName] || eventName,
      listener
    );
  }
  ...
  public publish<T>(
    ...
    if (!options || !options.name) {
      if ((<any>this.ipc).send !== undefined) {
        ...
        this.ipc
          .listeners(topic)
          .forEach((cb) => cb({ topic: topic }, message));
      }
    }
...
export class ElectronContainer extends WebContainerBase {
  ...
  public static readonly windowOptionsMap: PropertyMap = {
    ...
    node: {
      ...
      convert: (value: any, from: any, to: any) => {
        return Object.assign(to.webPreferences || {}, {
  ...
  public setOptions(options: any) {
    try {
      if (
        (!this.isRemote ||
          (options &&
            typeof options.isRemote !== "undefined" &&
            !options.isRemote)) &&
        !this.windowManager
      ) {
        this.windowManager = new ElectronWindowManager(
          this.app,
          this.internalIpc,
          this.browserWindow
        );
  ...
  protected registerNotificationsApi() {
    if (typeof this.globalWindow !== "undefined" && this.globalWindow) {
      ...
      this.globalWindow["Notification"] = class ElectronNotification extends (
        ContainerNotification
      ) {
  ...
  public getInfo(): Promise<string | undefined> {
    return Promise.resolve(
      `Electron/${this.electron.process.versions.electron} Chrome/${this.electron.process.versions.chrome}`
    );
  }
  ...
  public getMainWindow(): ContainerWindow {
    for (const window of this.browserWindow.getAllWindows()) {
      if (
        window[Container.windowOptionsPropertyKey] &&
        window[Container.windowOptionsPropertyKey].main
      ) {
  ...
  public createWindow(url: string, options?: any): Promise<ContainerWindow> {
    ...
    /*
        ...
        */
    if (this.isRemote) {
      electronWindow["name"] = (<any>this.internalIpc).sendSync(
        InternalMessageType.initialize,
        {
  ...
  public showNotification(title: string, options?: NotificationOptions) {
    ...
    if (options["notification"]) {
      notify.once("show", () =>
        notify.addListener("click", options["notification"]["onclick"])
      );
    }
  ...
  public getAllWindows(): Promise<ContainerWindow[]> {
    return Promise.resolve(
      this.browserWindow
        .getAllWindows()
        .map((window) => this.wrapWindow(window))
    );
  }
    ...
    return new Promise<PersistedWindowLayout>((resolve, reject) => {
      this.getAllWindows().then((windows) => {
        windows.forEach((window) => {
          const options =
            window.innerWindow[Container.windowOptionsPropertyKey];
...
export class ElectronWindowManager {
  ...
  public constructor(app?: any, ipc?: any, browserWindow?: any) {
    ...
    this.ipc.on(InternalMessageType.getOptions, (event: any, message: any) => {
      const { source: sourceId } = message;
      event.returnValue = this.browserWindow.fromId(sourceId)[
        Container.windowOptionsPropertyKey
      ];
    });
  }
  ...
  public initializeWindow(win: any, name: string, options: any) {
    ...
    if (
      options &&
      options.main &&
      (!("quitOnClose" in options) || options.quitOnClose)
    ) {
      win.on("closed", () => {
        this.app.quit();
      });
  ...
  public ungroupWindows(...windows: any[]) {
    ...
    this.groupBy<any>(this.browserWindow.getAllWindows(), "group")
      .filter((group) => group.key && group.values && group.values.length === 1)
      .forEach((group) => {
  ...
  private handleMove(win: any): void {
    ...
    if (win.group) {
      ...
      const groupedWindows = this.browserWindow
        .getAllWindows()
        .filter((window) => {
          return window.group === win.group && window.id !== win.id;
        });
      ...
      if (groupedWindows && groupedWindows.length > 0) {
        ...
        groupedWindows.forEach((groupedWindow) => {
          ...
          groupedWindow.setBounds(
            {
              x: targetBounds.x - diff.x,
              y: targetBounds.y - diff.y,
              width: targetBounds.width,
              height: targetBounds.height,
            },
            true
          );
          this.ignoredWindows.splice(
            this.ignoredWindows.indexOf(groupedWindow.id),
            1
          );
        });
      }
  ...
  private groupBy<T>(array: T[], groupBy: any): { key: any; values: T[] }[] {
    return array.reduce((accumulator, current) => {
      const key =
        groupBy instanceof Function ? groupBy(current) : current[groupBy];

...
class MockWindow extends MockEventEmitter {
  ...
  public getParentWindow(): any {
    return undefined;
  }
  public setParentWindow(parent: any) {}

  public getAllWindows(): any {
    return [new MockWindow(), new MockWindow("target")];
  }
...
describe("ElectronContainerWindow", () => {
  ...
  beforeEach(() => {
    ...
    container = new ElectronContainer(
      { BrowserWindow: { fromId(): any {} } },
      new MockIpc(),
      {}
    );
  ...
  describe("Window members", () => {
    ...
    it("minimize", (done) => {
      ...
      new ElectronContainerWindow(browserWindow, null)
        .minimize()
        .then(() => {
    ...
    describe("getState", () => {
      ...
      it("getState defined", (done) => {
        ...
        spyOn(innerWin.webContents, "executeJavaScript").and.returnValue(
    ...
    it("setParent calls underlying setParentWindow", (done) => {
      ...
      win.setParent(<any>mockParent).then(() => {
        expect(win.innerWindow.setParentWindow).toHaveBeenCalledWith(
          mockParent.innerWindow
        );
        done();
      });
    });
    ...
    it("getOptions sends synchronous ipc message", (done) => {
      ...
      win
        ...
        .then((options) => {
          expect(
            container.internalIpc.sendSync
          ).toHaveBeenCalledWith("desktopJS.window-getOptions", { source: 5 });
    ...
    describe("addListener", () => {
      it("calls underlying Electron window addListener", () => {
        ...
        expect(win.innerWindow.addListener).toHaveBeenCalledWith(
          "move",
      ...
      describe("beforeunload", () => {
        it("beforeunload attaches to underlying dom window", () => {
          ...
          spyOn(container, "getCurrentWindow").and.returnValue({
            id: "1",
          } as any);
    ...
    it("removeListener calls underlying Electron window removeListener", () => {
      ...
      expect(win.innerWindow.removeListener).toHaveBeenCalledWith(
        "move",
        jasmine.any(Function)
      );
    });
  ...
  describe("window grouping", () => {
    ...
    it("getGroup sends synchronous ipc message", (done) => {
      ...
      win
        ...
        .then((windows) => {
          expect(
            container.internalIpc.sendSync
          ).toHaveBeenCalledWith("desktopJS.window-getGroup", { source: 5 });
    ...
    it("getGroup invokes method directly in main process", () => {
      ...
      win = new ElectronContainerWindow(innerWin, container);
      (<any>container).windowManager = jasmine.createSpyObj("WindowManager", [
        "getGroup",
      ]);
    ...
    it("joinGroup sends ipc message", (done) => {
      ...
      win
        .joinGroup(targetWin)
        .then(() => {
          expect(container.internalIpc.send).toHaveBeenCalledWith(
            "desktopJS.window-joinGroup",
            {
              source: 1,
              target: 2,
            }
    ...
    it("leaveGroup sends ipc message", (done) => {
      ...
      win
        ...
        .then(() => {
          expect(
            container.internalIpc.send
          ).toHaveBeenCalledWith("desktopJS.window-leaveGroup", { source: 5 });
        })
        .then(done);
    });
...
describe("ElectronContainer", () => {
  ...
  it("createWindow", (done) => {
    ...
    container
      .createWindow("url", { x: "x", taskbar: false, node: true })
      .then(done);
    expect((<any>container).browserWindow).toHaveBeenCalledWith({
      x: "x",
      skipTaskbar: true,
  ...
  it("app browser-window-created fires Container window-created", (done) => {
    new ElectronContainer(electron, new MockMainIpc(), globalWindow, {
      isRemote: false,
    });
  ...
  it("createWindow on main process invokes ElectronWindowManager.initializeWindow", (done) => {
    ...
    (<any>container).windowManager = new ElectronWindowManager(
      {},
      new MockMainIpc(),
    ...
    expect(
      (<any>container).windowManager.initializeWindow
    ).toHaveBeenCalledWith(jasmine.any(Object), "name", options);
  });
  ...
  it("createWindow pulls nodeIntegration default from container", (done) => {
    ...
    container
      .createWindow("url", {})
      .then(() => {
        expect(container.browserWindow).toHaveBeenCalledWith({
          webPreferences: { nodeIntegration: true },
        });
      })
      .then(done);
  ...
  it("addTrayIcon", () => {
    ...
    container.addTrayIcon({ text: "text", icon: "icon" }, () => {}, [
  ...
  describe("notifications", () => {
    it("showNotification delegates to electron notification", () => {
      ...
      container.showNotification("title", <any>{
        onClick: () => {},
        notification: {},
      });
      expect(electron.Notification).toHaveBeenCalledWith({
        ...
        notification: jasmine.any(Object),
      });
    });
  ...
  describe("LoginItemSettings", () => {
    ...
    it("setOptions errors out on setLoginItemSettings", () => {
      electron.app.setLoginItemSettings = jasmine
        .createSpy()
        .and.callFake(() => {
    ...
    it("getOptions returns autoStartOnLogin status", (done) => {
      electron.app.getLoginItemSettings = jasmine
        .createSpy()
        .and.returnValue({ openAtLogin: true });
      container
        .getOptions()
        .then((result: any) => {
  ...
  describe("window management", () => {
    ...
    describe("getWindow", () => {
      it("getWindowById returns wrapped window", (done) => {
        spyOn(electron.BrowserWindow, "fromId").and.returnValue(
          new MockWindow()
        );
    ...
    it("buildLayout skips windows with persist false", (done) => {
      ...
      spyOn(container, "getAllWindows").and.returnValue(
        Promise.resolve([win1, win2])
      );
      container
        .buildLayout()
        .then((layout) => {
...
describe("ElectronWindowManager", () => {
  ...
  it("subscribed to ipc", () => {
    ...
    expect(ipc.on).toHaveBeenCalledWith(
      "desktopJS.window-leaveGroup",
      jasmine.any(Function)
    );
    expect(ipc.on).toHaveBeenCalledWith(
      "desktopJS.window-getGroup",
      jasmine.any(Function)
    );
    expect(ipc.on).toHaveBeenCalledWith(
      "desktopJS.window-getOptions",
      jasmine.any(Function)
    );
  });
  ...
  describe("main ipc handlers", () => {
    it("setname sets and returns supplied name", () => {
      ...
      const event: any = {};
      (<any>mgr).ipc.emit("desktopJS.window-initialize", event, {
        id: 1,
        name: "NewName",
      });
    ...
    it("joinGroup invokes groupWindows", () => {
      ...
      spyOn((<any>mgr).browserWindow, "fromId").and.callFake((id) =>
...
describe("ElectronDisplayManager", () => {
  ...
  beforeEach(() => {
    ...
    screen = jasmine.createSpyObj("screen", [
      "getPrimaryDisplay",
      "getAllDisplays",
...
describe("ElectronGlobalShortcutManager", () => {
  describe("invokes underlying Electron", () => {
    ...
    beforeEach(() => {
      electron = {
        globalShortcut: jasmine.createSpyObj("Electron", [
          "register",
          "unregister",
          "isRegistered",
          "unregisterAll",
    ...
    it("unregister", () => {
      ...
      expect(electron.globalShortcut.unregister).toHaveBeenCalledWith(
        "shortcut"
      );
    });

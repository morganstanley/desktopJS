...
export namespace Default {
  ...
  export class DefaultContainerWindow extends ContainerWindow {
    ...
    public getBounds(): Promise<Rectangle> {
      return new Promise<Rectangle>((resolve) => {
        resolve(
          new Rectangle(
            this.innerWindow.screenX,
            this.innerWindow.screenY,
            this.innerWindow.outerWidth,
            this.innerWindow.outerHeight
          )
        );
      });
    }
  ...
  export class DefaultMessageBus implements MessageBus {
      ...
      return new Promise<MessageBusSubscription>((resolve) => {
        const subscription: MessageBusSubscription = new MessageBusSubscription(
          ...
          (event: any) => {
            ...
            if (
              source === DefaultMessageBus.messageSource &&
              topic === receivedTopic
            ) {
        ...
        if (
          this.container.globalWindow &&
          this.container.globalWindow.addEventListener
        ) {
          this.container.globalWindow.addEventListener(
            "message",
    ...
    unsubscribe(subscription: MessageBusSubscription): Promise<void> {
      return Promise.resolve(
        this.container.globalWindow.removeEventListener(
          "message",
          subscription.listener
        )
      );
      ...
      const windows: Window[] = this.container.globalWindow
        ? this.container.globalWindow[DefaultContainer.windowsPropertyKey] ||
          (this.container.globalWindow.opener &&
            this.container.globalWindow.opener[
              DefaultContainer.windowsPropertyKey
            ])
        : [];
      ...
      if (windows) {
        for (const key in windows) {
          ...
          if (
            options &&
            options.name &&
            options.name !== win[DefaultContainer.windowNamePropertyKey]
          ) {
            continue;
          }
  ...
  export class DefaultContainer extends WebContainerBase {
    ...
    public windowOptionsMap: PropertyMap =
    ...
    public constructor(win?: Window) {
      ...
      if (
        this.globalWindow &&
        !(DefaultContainer.windowsPropertyKey in this.globalWindow)
      ) {
        this.globalWindow[DefaultContainer.windowsPropertyKey] = {
          root: this.globalWindow,
        };
        this.globalWindow[
          DefaultContainer.windowNamePropertyKey
        ] = this.globalWindow[DefaultContainer.windowUuidPropertyKey] =
          DefaultContainer.rootWindowUuid;
      }
    ...
    protected getWindowOptions(options?: any): any {
      return ObjectTransform.transformProperties(
        options,
    ...
    public createWindow(url: string, options?: any): Promise<ContainerWindow> {
      ...
      if (newOptions) {
        for (const prop in newOptions) {
          features =
            (features ? features : "") + prop + "=" + newOptions[prop] + ",";
        }
      ...
      try {
        ...
        this.log(
          "warn",
          `Error proprogating properties to new window, '${e.message}'`
        );
      }
      ...
      this.emit("window-created", {
        ...
        name: "window-created",
        window: newWindow,
        windowId: windowReachable ? newWindow.id : undefined,
        windowName: newOptions.name,
      });
    ...
    public getAllWindows(): Promise<ContainerWindow[]> {
      return new Promise<ContainerWindow[]>((resolve) => {
        ...
        const trackedWindows = this.globalWindow[
          DefaultContainer.windowsPropertyKey
        ];
    ...
    public getWindowByName(name: string): Promise<ContainerWindow | null> {
      return new Promise<ContainerWindow>((resolve) => {
        ...
        for (const key in trackedWindows) {
          if (
            trackedWindows[key][DefaultContainer.windowNamePropertyKey] === name
          ) {
        ...
        this.getAllWindows().then((windows) => {
          windows.forEach((window) => {
            ...
            try {
                ...
                new Promise<void>(async (innerResolve) => {
                  if (this.globalWindow !== nativeWin) {
                    layout.windows.push({
                      name: window.name,
                      url:
                        nativeWin && nativeWin.location
                          ? nativeWin.location.toString()
                          : undefined,
                      id: window.id,
                      bounds: {
                        x: nativeWin.screenX,
                        y: nativeWin.screenY,
                        width: nativeWin.innerWidth,
                        height: nativeWin.innerHeight,
                      },
                      options: options,
              ...
              this.log(
                "warn",
                `Error while accessing window so skipping, '${e.message}'`
              );
            }
          });
  ...
  class DefaultDisplayManager implements ScreenManager {
    ...
    public getPrimaryDisplay(): Promise<Display> {
      return new Promise<Display>((resolve) => {
        ...
        display.bounds = new Rectangle(
          (<any>this.window.screen).availLeft,
    ...
    public getMousePosition(): Promise<Point> {
      return new Promise<Point>((resolve) => {
        resolve({
          x: (<any>this.window.event).screenX,
          y: (<any>this.window.event).screenY,
        });
      });
    }
  }

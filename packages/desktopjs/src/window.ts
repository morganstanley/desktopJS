...
export abstract class ContainerWindow extends EventEmitter {
   ...
   */
  protected abstract detachListener(
    eventName: WindowEventType,
    listener: (event: EventArgs) => void
  ): void;
  ...
  public static addListener(
    ...
    EventEmitter.addListener(
      ContainerWindow.staticEventScopePrefix + eventName,
      listener
    );
  }
...
export class GroupWindowManager {
  ...
  public attach(win?: ContainerWindow) {
    if (win) {
      win.addListener(
        isOpenFin() ? <WindowEventType>"minimized" : "minimize",
        (e) => {
          if (
            this.windowStateTracking & WindowStateTracking.Main &&
            this.container.getMainWindow().id === e.sender.id
          ) {
      ...
      win.addListener(
        isOpenFin() ? <WindowEventType>"restored" : "restore",
        (e) => {
          if (
            this.windowStateTracking & WindowStateTracking.Main &&
            this.container.getMainWindow().id === e.sender.id
...
export class SnapAssistWindowManager extends GroupWindowManager {
  ...
  protected onAttached(win: ContainerWindow) {
    ...
    if (isOpenFin()) {
      ...
      win.addListener(<WindowEventType>"disabled-frame-bounds-changed", () =>
        this.onMoved(win)
      );
      win.addListener(<WindowEventType>"frame-enabled", () =>
        win.innerWindow.disableFrame()
      );
    } else {
  ...
  protected onMoving(e: any) {
    ...
    e.sender.getOptions().then((senderOptions) => {
      if (
        senderOptions &&
        typeof senderOptions.snap !== "undefined" &&
        senderOptions.snap === false
      ) {
        return;
      }
        ...
        getBounds.then((bounds) => {
          ...
          this.container.getAllWindows().then((windows) => {
            ...
            Promise.all(promises).then((responses) => {
              ...
              for (const target of responses.filter(
                (response) =>
                  !(
                    response.options &&
                    typeof response.options.snap !== "undefined" &&
                    response.options.snap === false
                  )
              )) {
                snapHint = this.getSnapBounds(
                  snapHint || bounds,
                  target.bounds
                );
                if (snapHint) {
  ...
  protected onMoved(win: ContainerWindow) {
    if (this.autoGrouping) {
      ...
      this.targetGroup.forEach((target) =>
        groupCallbacks.push(
          new Promise<WindowGroupStatus>((resolve) => {
            target
              .getGroup()
              .then((windows) =>
                resolve({ window: target, isGrouped: windows.length > 0 })
              );
          })
        )
      );
  ...
  private getSnapBounds(r1: Rectangle, r2: Rectangle): Rectangle {
    ...
    if (this.isVerticallyAligned(r1, r2)) {
      ...
      isTopToBottom =
        Math.abs(r1.y - (r2.bottom - this.snapOffset)) < this.snapThreshold;
      isTopToTop = Math.abs(r1.y - r2.y) < this.snapThreshold;

      // Bottom edge of r1 is within threshold of top edge of r2
      isBottomToTop =
        Math.abs(r1.bottom - this.snapOffset - r2.y) < this.snapThreshold;

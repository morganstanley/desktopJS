...
export abstract class Container
  ...
  implements ContainerWindowManager, ContainerNotificationManager {
  ...
  public abstract getWindowByName(
    name: string
  ): Promise<ContainerWindow | null>;
  ...
  public static addListener(
    ...
    EventEmitter.addListener(
      Container.staticEventScopePrefix + eventName,
      listener
    );
  }
...
export abstract class ContainerBase extends Container {
  ...
  public storage: Storage =
    typeof window !== "undefined" && window ? window.localStorage : undefined;

  protected getLayoutFromStorage(name: string): PersistedWindowLayout {
    return JSON.parse(this.storage.getItem(ContainerBase.layoutsPropertyKey))[
      name
    ];
  }
  ...
  protected saveLayoutToStorage(name: string, layout: PersistedWindowLayout) {
    const layouts: any =
      JSON.parse(this.storage.getItem(ContainerBase.layoutsPropertyKey)) || {};
    ...
    this.storage.setItem(
      ContainerBase.layoutsPropertyKey,
  ...
  protected deleteLayoutFromStorage(name: string) {
    ...
    if (layout) {
      ...
      this.emit("layout-deleted", {
        sender: this,
        name: "layout-deleted",
        layoutName: layout.name,
      });
      Container.emit("layout-deleted", {
        name: "layout-deleted",
        layoutName: layout.name,
      });
    }
      ...
      this.closeAllWindows(true).then(() => {
        const layoutToLoad =
          typeof layout === "string"
            ? <PersistedWindowLayout>this.getLayoutFromStorage(layout)
            : layout;
        if (layoutToLoad && layoutToLoad.windows) {
          ...
          Promise.all(promises).then((windows) => {
            ...
            windows.forEach((window) => {
              const matchingWindow = layoutToLoad.windows.find(
                (win) => win.name === window.name
              );
              if (matchingWindow && matchingWindow.state && window.setState) {
                window
                  .setState(matchingWindow.state)
                  .catch((e) =>
                    this.log("error", "Error invoking setState: " + e)
                  );
              }
            ...
            groupMap.forEach((targets, window) => {
              targets.forEach((target) => {
                this.getWindowByName(
                  layoutToLoad.windows.find((win) => win.id === target).name
                ).then((targetWin) => {
          ...
          this.emit("layout-loaded", {
            sender: this,
            name: "layout-loaded",
            layout: layoutToLoad,
            layoutName: layoutToLoad.name,
          });
          Container.emit("layout-loaded", {
            name: "layout-loaded",
            layout: layoutToLoad,
            layoutName: layoutToLoad.name,
          });

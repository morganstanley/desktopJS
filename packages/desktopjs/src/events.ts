...
export class EventEmitter {
  ...
  private static staticEventListeners: Map<
    string,
    ((event: EventArgs) => void)[]
  > = new Map();

  private wrappedListeners: Map<
    (event: EventArgs) => void,
    (event: EventArgs) => void
  > = new Map();
  ...
  protected registerAndWrapListener(
    eventName: string,
    listener: (event: EventArgs) => void
  ): (event: EventArgs) => void {
  ...
  protected unwrapAndUnRegisterListener(
    listener: (event: EventArgs) => void
  ): (event: EventArgs) => void {
  ...
  public static addListener(
    ...
    (this.staticEventListeners[eventName] =
      this.staticEventListeners[eventName] || []).push(listener);
  }
  ...
  public static emit(
    ...
    if (ipc && ipc.publish) {
      ipc.publish(EventEmitter.staticEventName, {
        eventName: eventName,
        eventArgs: eventArgs,
      });
    } else {

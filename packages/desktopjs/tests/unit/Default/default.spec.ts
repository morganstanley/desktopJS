...
describe("DefaultContainerWindow", () => {
  ...
  describe("getState", () => {
    ...
    it("getState defined", (done) => {
      ...
      spyOn(win.innerWindow, "getState").and.returnValue(
  ...
  it("minimize", (done) => {
    ...
    new Default.DefaultContainerWindow(innerWindow)
      .minimize()
      .then(() => {
  ...
  it("getOptions", async (done) => {
    const win = await new Default.DefaultContainer(
      <any>new MockWindow()
    ).createWindow("url", { a: "foo" });
    win
      .getOptions()
      .then((options) => {
  ...
  describe("window grouping", () => {
    it("allowGrouping is false", () => {
      expect(new Default.DefaultContainerWindow(null).allowGrouping).toEqual(
        false
      );
    });

    it("getGroup returns empty array", (done) => {
      new Default.DefaultContainerWindow(null)
        .getGroup()
        .then((windows) => {
  ...
  it("nativeWindow returns wrapped window", () => {
    const innerWindow = {};
    const nativeWindow = new Default.DefaultContainerWindow(innerWindow)
      .nativeWindow;
...
describe("DefaultContainer", () => {
  ...
  it("getOptions returns autoStartOnLogin as false", (done) => {
    ...
    container
      .getOptions()
      .then(
        (result: { autoStartOnLogin: boolean }) => {},
        (result) => {
  ...
  describe("createWindow", () => {
    ...
    it("Window is added to windows", (done) => {
      container.createWindow("url").then((win) => {
        ...
        expect(
          newWin[Default.DefaultContainer.windowUuidPropertyKey]
        ).toBeDefined();
        expect(
          newWin[Default.DefaultContainer.windowsPropertyKey]
        ).toBeDefined();
        expect(
          newWin[Default.DefaultContainer.windowsPropertyKey][
            newWin[Default.DefaultContainer.windowUuidPropertyKey]
  ...
  describe("window management", () => {
    it("getAllWindows returns wrapped native windows", (done) => {
      ...
      container.getAllWindows().then((wins) => {
        ...
        wins.forEach((win) =>
          expect(win instanceof Default.DefaultContainerWindow).toBeTruthy(
            "Window is not of type DefaultContainerWindow"
          )
        );
        done();
      });
    ...
    describe("getWindow", () => {
      ...
      it("getWindowById returns wrapped window", (done) => {
        ...
        container.getWindowById("1").then((win) => {
          expect(win).toBeDefined();
          expect(win.innerWindow).toEqual(
            window[Default.DefaultContainer.windowsPropertyKey]["1"]
          );
          done();
        });
      });
      ...
      it("getWindowByName returns wrapped window", (done) => {
        ...
        window[Default.DefaultContainer.windowsPropertyKey]["1"][
          Default.DefaultContainer.windowNamePropertyKey
        ] = "Name";
    ...
    it("buildLayout skips windows with persist false", (done) => {
      ...
      window[Default.DefaultContainer.windowsPropertyKey]["2"][
        Default.DefaultContainer.windowNamePropertyKey
      ] = "win2";
...
describe("DefaultMessageBus", () => {
  ...
  it("listener callback attached", (done) => {
    ...
    bus.subscribe("topic", handler).then((subscriber) => {
      subscriber.listener({
        origin: "origin",
        data: { source: "desktopJS", topic: "topic", message: "message" },
      });
    });
  });

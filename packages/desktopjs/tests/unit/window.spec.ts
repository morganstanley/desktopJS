...
describe("static events", () => {
  ...
  it("addListener adds callback to listeners", () => {
    ...
    ContainerWindow.addListener("window-created", (event: EventArgs) => {
      /* empty */
    });
  ...
  it("emit invokes ipc publish", () => {
    ...
    expect(container.ipc.publish).toHaveBeenCalledWith(
      "desktopJS.static-event",
      { eventName: "containerwindow-" + args.name, eventArgs: args }
    );
  });
});
...
describe("Rectangle", () => {
  ...
  it("right is calculated as x + width", () => {
    ...
    expect(
      Rectangle.getRight(<Rectangle>{ x: 1, y: 2, width: 3, height: 4 })
    ).toEqual(4);
  });
  ...
  it("bottom is calculated as top + height", () => {
    ...
    // check duck typing static method helper
    expect(
      Rectangle.getBottom(<Rectangle>{ x: 1, y: 2, width: 3, height: 4 })
    ).toEqual(6);
  });
});
...
describe("GroupWindowManager", () => {
  ...
  it("overrides read from options", () => {
    const mgr = new GroupWindowManager(null, {
      windowStateTracking: WindowStateTracking.Main,
    });
    expect(mgr.windowStateTracking).toEqual(WindowStateTracking.Main);
  });

  it("Minimize with Main", () => {
    const innerWin = jasmine.createSpyObj("innerwindow", [
      "minimize",
      "addListener",
    ]);
    const container = jasmine.createSpyObj("container", [
      "getAllWindows",
      "getMainWindow",
    ]);
    ...
    const mgr = new GroupWindowManager(container, {
      windowStateTracking: WindowStateTracking.Main,
    });
    mgr.attach(win);
  ...
  it("Minimize with Group", () => {
    ...
    container.getAllWindows.and.returnValue(Promise.resolve([win]));
    const mgr = new GroupWindowManager(container, {
      windowStateTracking: WindowStateTracking.Group,
    });
    mgr.attach(win);
    win.emit("minimize", { name: "minimize", sender: win });
...
describe("SnapAssistWindowManager", () => {
  ...
  it("attach invoked on create", () => {
    ...
    const mgr = new SnapAssistWindowManager(null);
    expect(ContainerWindow.listeners("window-created").length).toEqual(
      count + 1
    );
  });
  ...
  it("onAttached hooks on Electron wndproc when available", () => {
    ...
    const mgr = new SnapAssistWindowManager(null);
    (<any>mgr).onAttached(win);
    expect(innerWin.hookWindowMessage).toHaveBeenCalledWith(
      0x0232,
  ...
  it("move handler", (done) => {
    const win = jasmine.createSpyObj("window", [
      "addListener",
      "getGroup",
      "getBounds",
      "setBounds",
    ...
    const win2 = jasmine.createSpyObj("targetWindow", [
      "addListener",
      "getBounds",
      "getOptions",
    ]);
    Object.defineProperty(win2, "id", { value: "2" });
    win2.getBounds.and.returnValue(
      Promise.resolve(new Rectangle(52, 0, 50, 50))
    );
  ...
  describe("getSnapBounds", () => {
    ...
    describe("left edge to right edge", () => {
      it("within threshold", () => {
        ...
        expect((<any>mgr).getSnapBounds(r1, r2)).toEqual(
          new Rectangle(r2.right - mgr.snapOffset, 0, 50, 50)
        );
      });
    ...
    describe("right edge to left edge", () => {
      it("within threshold", () => {
        ...
        expect((<any>mgr).getSnapBounds(r1, r2)).toEqual(
          new Rectangle(r2.x - r1.width + mgr.snapOffset, 0, 50, 50)
        );
      });
    ...
    describe("bottom edge to top edge", () => {
      it("within threshold", () => {
        ...
        expect((<any>mgr).getSnapBounds(r1, r2)).toEqual(
          new Rectangle(
            0,
            r2.y - (r1.height - Math.floor(mgr.snapOffset / 2)),
            50,
            50
          )
        );
    ...
    describe("top edge to bottom edge", () => {
      it("within threshold", () => {
        ...
        expect((<any>mgr).getSnapBounds(r1, r2)).toEqual(
          new Rectangle(0, r2.bottom - Math.floor(mgr.snapOffset / 2), 50, 50)
        );
      });

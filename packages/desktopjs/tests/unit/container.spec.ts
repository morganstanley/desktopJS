...
export class TestContainer extends ContainerBase {
  getMainWindow(): ContainerWindow {
    const win = jasmine.createSpyObj("ContainerWindow", [
      "setBounds",
      "getState",
      "setState",
    ]);
  ...
  public getWindowById(): Promise<ContainerWindow> {
    return Promise.resolve(undefined);
  }
  ...
  public saveLayout(): Promise<PersistedWindowLayout> {
    return Promise.resolve(undefined);
  }
...
describe("container", () => {
  ...
  describe("ContainerBase", () => {
    ...
    describe("showNotification", () => {
      it("Throws Not implemented", () => {
        expect(() =>
          container.showNotification(<any>new NotificationOptions())
    ...
    describe("window management", () => {
      beforeEach(() => {
        spyOn(container, "createWindow").and.returnValue(
          jasmine.createSpyObj("window", ["joinGroup"])
        );
      });
      ...
      it("loadLayout by name", (done) => {
        container.loadLayout("Test").then((layout) => {
          ...
          expect(container.createWindow).toHaveBeenCalledWith("url", {
            name: "1",
          });
          done();
        });
      ...
      it("loadLayout with no windows rejects", (done) => {
        container
          .loadLayout(new PersistedWindowLayout("Test"))
          .catch((error) => {
            expect(error).toEqual("Layout does not exist or is invalid");
          })
          .then(done);
      ...
      it("loadLayout with poorly constructed layout still creates windows", async () => {
        ...
        const layout = await container.loadLayout(layoutToLoad);
        expect(container.createWindow).toHaveBeenCalledTimes(1);
        expect(container.createWindow).toHaveBeenCalledWith(undefined, {
          name: undefined,
        });
        expect(layout).toBeDefined();

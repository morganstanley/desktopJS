...
class MockWindow {
  ...
  public nativeWindow: Window = jasmine.createSpyObj("window", [
    "location",
    "getState",
    "setState",
  ]);
...
describe("OpenFinContainerWindow", () => {
  ...
  it("load", (done) => {
    ...
    win
      .load("url")
      .then(() => {
        expect(innerWin.navigate).toHaveBeenCalledWith(
          "url",
  ...
  describe("setState", () => {
    ...
    it("setState defined", (done) => {
      ...
      win
        .setState(mockState)
        .then(() => {
          expect(innerWin.nativeWindow.setState).toHaveBeenCalledWith(
            mockState
          );
        })
        .then(done);
  ...
  describe("getSnapshot", () => {
    ...
    it("getSnapshot propagates internal error to promise reject", (done) => {
      spyOn(innerWin, "getSnapshot").and.callFake((callback, reject) =>
        reject("Error")
      );
      let success = false;
    ...
    it("setBounds sets underlying window position", (done) => {
      ...
      win
        .setBounds(<any>{ x: 0, y: 1, width: 2, height: 3 })
        .then(() => {
          expect(win.innerWindow.setBounds).toHaveBeenCalledWith(
            0,
            1,
            2,
            3,
    ...
    describe("getOptions", () => {
      it("getOptions invokes underlying getOptions and returns undefined customData", (done) => {
        spyOn(win.innerWindow, "getOptions").and.callFake((callback) =>
          callback({})
        );
        win
          .getOptions()
  ...
  describe("addListener", () => {
    it("addListener calls underlying OpenFin window addEventListener with mapped event name", () => {
      ...
      expect(win.innerWindow.addEventListener).toHaveBeenCalledWith(
        "bounds-changing",
        jasmine.any(Function)
      );
    });
    ...
    it("resize wraps filtered bounds-changing", (done) => {
      spyOn(win.innerWindow, "addEventListener").and.callFake(
        (eventName, listener) => {
  ...
  describe("removeListener", () => {
    it("removeListener calls underlying OpenFin window removeEventListener with mapped event name", () => {
      ...
      expect(win.innerWindow.removeEventListener).toHaveBeenCalledWith(
        "bounds-changing",
        jasmine.any(Function)
      );
    });
  ...
  describe("window grouping", () => {
    ...
    it("joinGroup invokes underlying joinGroup", (done) => {
      ...
      win
        .joinGroup(window)
        .then(() => {
          expect(innerWin.joinGroup).toHaveBeenCalledWith(
            window.innerWindow,
            jasmine.any(Function),
            jasmine.any(Function)
          );
...
describe("OpenFinContainer", () => {
  ...
  it("getInfo invokes underlying getRvmInfo and getRuntimeInfo", (done) => {
    const system = jasmine.createSpyObj("system", [
      "getRvmInfo",
      "getRuntimeInfo",
      "log",
    ]);
  ...
  describe("ctor options", () => {
    describe("registerUser", () => {
      ...
      beforeEach(() => {
        desktop = jasmine.createSpyObj("desktop", [
          "Application",
          "InterApplicationBus",
          "GlobalHotkey",
        ]);
        app = jasmine.createSpyObj("application", [
          "getCurrent",
          "registerUser",
          "addEventListener",
          "setShortcuts",
        ...
        Object.defineProperty(desktop, "InterApplicationBus", {
          value: new MockInterApplicationBus(),
        });
  ...
  describe("log", () => {
    beforeEach(() => {
      Object.defineProperty(desktop, "System", {
        value: jasmine.createSpyObj("System", ["log"]),
      });
    });

    it("debug", () => {
      container.log("debug", "message");
      expect(desktop.System.log).toHaveBeenCalledWith(
        "debug",
        "message",
        jasmine.any(Function),
        jasmine.any(Function)
  ...
  describe("createWindow", () => {
    beforeEach(() => {
      spyOn(desktop, "Window").and.callFake(
        ...
        expect(desktop.Window).toHaveBeenCalledWith(
          {
            autoShow: true,
            url: "url",
            name: jasmine.stringMatching(/\w{8}-\w{4}-\w{4}-\w{4}-\w{12}/),
    ...
    it("createWindow defaults", (done) => {
      spyOn<any>(container, "ensureAbsoluteUrl").and.returnValue(
        "absoluteIcon"
      );
      const options = {
        x: "x",
  ...
  describe("window management", () => {
    ...
    it("saveLayout invokes underlying saveLayoutToStorage", (done) => {
      ...
      container
        ...
        .then((layout) => {
          ...
          expect((<any>container).saveLayoutToStorage).toHaveBeenCalledWith(
            "Test",
            layout
          );
          done();
    ...
    it("getOptions returns autoStartOnLogin status", (done) => {
      ...
      const current = app.getCurrent();
      spyOn(app, "getCurrent").and.callThrough();
      spyOn(current, "getShortcuts").and.callFake((callback) =>
  ...
  describe("notifications", () => {
    it("showNotification passes message and invokes underlying notification api", () => {
      ...
      container.showNotification("title", {
        body: "Test message",
        url: "notification.html",
      });
      expect(desktop.Notification).toHaveBeenCalledWith({
        url: "notification.html",
  ...
  it("getMenuItemHtml with icon has embedded icon in span", () => {
    ...
    expect(menuItemHtml).toEqual(
      `<li class="context-menu-item" onclick="fin.desktop.InterApplicationBus.send('uuid', null, 'TrayIcon_ContextMenuClick_${container.uuid}', { id: '${menuItem.id}' });this.close()"><span><img align="absmiddle" class="context-menu-image" src="${menuItem.icon}" /></span>Label</li>`
    );
  });
  ...
  it("getMenuItemHtml with no icon has nbsp; in span", () => {
    ...
    const menuItemHtml: string = (<any>container).getMenuItemHtml(menuItem);
    expect(menuItemHtml).toEqual(
      `<li class="context-menu-item" onclick="fin.desktop.InterApplicationBus.send('uuid', null, 'TrayIcon_ContextMenuClick_${container.uuid}', { id: '${menuItem.id}' });this.close()"><span>&nbsp;</span>Label</li>`
    );
  });
  ...
  it("addTrayIcon invokes underlying setTrayIcon", () => {
    ...
    expect(MockDesktop.application.setTrayIcon).toHaveBeenCalledWith(
      "icon",
      jasmine.any(Function),
      jasmine.any(Function),
      jasmine.any(Function)
...
describe("OpenFinMessageBus", () => {
  ...
  beforeEach(() => {
    bus = new OpenFinMessageBus(
      <any>(mockBus = new MockInterApplicationBus()),
      "uuid"
    );
  });
  ...
  it("unsubscribe with options invokes underlying unsubscribe", (done) => {
    spyOn(mockBus, "unsubscribe").and.callThrough();
    const sub: MessageBusSubscription = new MessageBusSubscription(
      "topic",
      callback,
      { uuid: "uuid", name: "name" }
    );
...
describe("OpenFinDisplayManager", () => {
  ...
  beforeEach(() => {
    ...
    system = jasmine.createSpyObj("system", [
      "getMonitorInfo",
      "getMousePosition",
    ]);
    Object.defineProperty(desktop, "Application", { value: app });
    ...
    system.getMousePosition.and.callFake((callback) =>
      callback({ left: 1, top: 2 })
    );
    system.getMonitorInfo.and.callFake((callback) =>
      callback({
        primaryMonitor: {
          deviceId: "deviceId1",
          name: "name1",
...
describe("OpenfinGlobalShortcutManager", () => {
  ...
  it("Unavailable in OpenFin is unavailable on container", () => {
    ...
    expect(OpenFinContainer.prototype.log).toHaveBeenCalledWith(
      "warn",
  ...
  describe("invokes underlying OpenFin", () => {
    beforeEach(() => {
      desktop.GlobalHotkey = jasmine.createSpyObj("GlobalHotKey", [
        "register",
        "unregister",
        "isRegistered",
        "unregisterAll",
    ...
    it("register", () => {
      ...
      expect(desktop.GlobalHotkey.register).toHaveBeenCalledWith(
        "shortcut",
        jasmine.any(Function),
        jasmine.any(Function),
        jasmine.any(Function)
    ...
    it("unregister", () => {
      ...
      expect(desktop.GlobalHotkey.unregister).toHaveBeenCalledWith(
        "shortcut",
        jasmine.any(Function),
        jasmine.any(Function)
      );
    ...
    it("isRegistered", (done) => {
      desktop.GlobalHotkey.isRegistered.and.callFake(
        (shortcut, resolve, reject) => resolve(true)
      );
      container.globalShortcut
        .isRegistered("shortcut")
        .then(() => {
          expect(desktop.GlobalHotkey.isRegistered).toHaveBeenCalledWith(
            "shortcut",
            jasmine.any(Function),
            jasmine.any(Function)
          );
    ...
    it("unregisterAll", () => {
      ...
      expect(desktop.GlobalHotkey.unregisterAll).toHaveBeenCalledWith(
        jasmine.any(Function),
        jasmine.any(Function)
      );
    });

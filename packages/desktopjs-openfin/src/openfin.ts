...
export class OpenFinContainerWindow extends ContainerWindow {
  ...
  public joinGroup(target: ContainerWindow): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      this.innerWindow.joinGroup(
        target.innerWindow,
        () => {
          ContainerWindow.emit("window-joinGroup", {
            name: "window-joinGroup",
            windowId: this.id,
  ...
  public leaveGroup(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this.innerWindow.leaveGroup(() => {
        ContainerWindow.emit("window-leaveGroup", {
          name: "window-leaveGroup",
          windowId: this.id,
        });
        resolve();
      }, reject);
  ...
  public getOptions(): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.innerWindow.getOptions(
        (options) =>
          resolve(
            options.customData ? JSON.parse(options.customData) : undefined
          ),
        reject
      );
    });
  ...
  public getState(): Promise<any> {
    return new Promise<any>((resolve) => {
      this.nativeWindow && (<any>this.nativeWindow).getState
  ...
  public setState(state: any): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      this.emit("state-changed", <EventArgs>{
        name: "state-changed",
        sender: this,
        state: state,
      });
      ContainerWindow.emit("state-changed", <WindowEventArgs>{
        name: "state-changed",
        windowId: this.id,
        state: state,
      });
  ...
  protected attachListener(
    ...
    if (eventName === "beforeunload") {
      ...
      this.innerWindow.addEventListener(
        windowEventMap[eventName] || eventName,
        listener
      );
    }
  }
  ...
  protected detachListener(
    eventName: string,
    listener: (...args: any[]) => void
  ): any {
    this.innerWindow.removeEventListener(
      windowEventMap[eventName] || eventName,
      listener
    );
  }
...
export class OpenFinMessageBus implements MessageBus {
  ...
  public subscribe<T>(
    ...
    return new Promise<MessageBusSubscription>((resolve, reject) => {
      const subscription: MessageBusSubscription = new MessageBusSubscription(
        topic,
      ...
      this.bus.subscribe(
        (options && options.uuid) || "*", // senderUuid
  ...
  public publish<T>(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if ((options && options.uuid) || (options && options.name)) {
        this.bus.send(
          options.uuid || this.uuid, // destinationUuid
...
export class OpenFinContainer extends WebContainerBase {
  ...
  private static readonly notificationGuid: string =
    "A21B62E0-16B1-4B10-8BE3-BBB6B489D862";
  ...
  public notificationOptionsMap: PropertyMap =
    OpenFinContainer.notificationOptionsMap;

  public static menuHtml = `<html>
        <head>
            <style>
  ...
  public constructor(desktop?: any, win?: Window, options?: any) {
    ...
    this.desktop.Application.getCurrent().addEventListener(
      "window-created",
      (event: any) => {
        this.emit("window-created", {
          ...
          name: "window-created",
          windowId: event.name,
          windowName: event.name,
        });
        Container.emit("window-created", {
          name: "window-created",
          windowId: event.name,
          windowName: event.name,
        });
        ContainerWindow.emit("window-created", {
          name: "window-created",
          windowId: event.name,
          windowName: event.name,
        });
  ...
  public setOptions(options: any) {
    if (options && options.userName && options.appName) {
      this.desktop.Application.getCurrent().registerUser(
        options.userName,
        options.appName
      );
    }
    ...
    if (options && options.autoStartOnLogin) {
      this.desktop.Application.getCurrent().setShortcuts({
        systemStartup: options.autoStartOnLogin,
      });
    }
  ...
  private isAutoStartEnabledAtLogin(): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      this.desktop.Application.getCurrent().getShortcuts(
        (config: any) => resolve(config.systemStartup),
        reject
      );
    });
  }
  ...
  protected createMessageBus(): MessageBus {
    return new OpenFinMessageBus(
      this.desktop.InterApplicationBus,
  ...
  protected registerNotificationsApi() {
    if (typeof this.globalWindow !== "undefined" && this.globalWindow) {
      ...
      this.globalWindow["Notification"] = class OpenFinNotification extends (
        ContainerNotification
      ) {
  ...
  public getMainWindow(): ContainerWindow {
    if (!this.mainWindow) {
      this.mainWindow = this.wrapWindow(
        this.desktop.Application.getCurrent().getWindow()
      );
    }
  ...
  protected getWindowOptions(options?: any): any {
    const newOptions = ObjectTransform.transformProperties(
      options,
      this.windowOptionsMap
    );
    ...
    if (
      !("saveWindowState" in newOptions) &&
      ("defaultLeft" in newOptions || "defaultTop" in newOptions)
    ) {
  ...
  public createWindow(url: string, options?: any): Promise<ContainerWindow> {
    ...
    return new Promise<ContainerWindow>((resolve, reject) => {
      const ofWin = new this.desktop.Window(
        newOptions,
        (win) => {
  ...
  public showNotification(title: string, options?: NotificationOptions) {
    const msg = new this.desktop.Notification(
      ObjectTransform.transformProperties(options, this.notificationOptionsMap)
    );
  }
  ...
  protected getMenuItemHtml(item: MenuItem) {
    const imgHtml: string = item.icon
      ? `<span><img align="absmiddle" class="context-menu-image" src="${this.ensureAbsoluteUrl(
          item.icon
        )}" /></span>`
      : "<span>&nbsp;</span>";

    return (
      `<li class="context-menu-item" onclick="fin.desktop.InterApplicationBus.send('${
        (<any>this.desktop.Application.getCurrent()).uuid
      }` +
  ...
  private showMenu(
    ...
    const contextMenu = new this.desktop.Window(
      ...
      () => {
        ...
        const contextMenuElement: HTMLElement = win.document.getElementById(
          "contextMenu"
        );
        ...
        const left =
          x + width > monitorInfo.primaryMonitor.monitorRect.right
            ? x - width - OpenFinContainer.trayIconMenuLeftOffset
            : x;
        const top =
          y + height > monitorInfo.primaryMonitor.monitorRect.bottom
  ...
  public addTrayIcon(
    ...
    this.desktop.Application.getCurrent().setTrayIcon(
      this.ensureAbsoluteUrl(details.icon),
      (clickInfo) => {
        if (clickInfo.button === 0 && listener) {
          // Passthrough left click to addTrayIcon listener callback
          listener();
        } else if (clickInfo.button === 2) {
          // handle right click ourselves to display context menu
          this.showMenu(
            clickInfo.x + OpenFinContainer.trayIconMenuLeftOffset,
      ...
      () => {
        ...
        this.desktop.InterApplicationBus.subscribe(
          (<any>this.desktop.Application.getCurrent()).uuid,
          "TrayIcon_ContextMenuClick_" + this.uuid,
          (message, uuid) => {
        ...
        windows.forEach((window) => {
          windowClosing.push(
            new Promise<void>((innerResolve, innerReject) =>
              window.close(true, innerResolve, innerReject)
            )
          );
        });
  ...
  public buildLayout(): Promise<PersistedWindowLayout> {
    ...
    return new Promise<PersistedWindowLayout>(async (resolve, reject) => {
      ...
      windows
        .filter(
          (window) =>
            window.name !== "queueCounter" &&
            !window.name.startsWith(OpenFinContainer.notificationGuid)
        )
        .forEach((djsWindow) => {
          promises.push(
            // eslint-disable-next-line no-async-promise-executor
            new Promise<void>(async (innerResolve, innerReject) => {
              ...
              window.getBounds((bounds) => {
                window.getOptions((options) => {
                  ...
                  const customData: any = options.customData
                    ? JSON.parse(options.customData)
                    : undefined;
                  if (
                    customData &&
                    "persist" in customData &&
                    !customData.persist
                  ) {
                    innerResolve();
                  } else {
                    ...
                    window.getGroup((group) => {
                      layout.windows.push({
                        name: window.name,
                        id: window.name,
                        url: window.getNativeWindow()
                          ? window.getNativeWindow().location.toString()
                          : options.url,
...
class OpenFinDisplayManager implements ScreenManager {
  ...
  createDisplay(monitorDetails: any) {
    ...
    display.bounds = new Rectangle(
      monitorDetails.monitorRect.left,
    ...
    display.workArea = new Rectangle(
      monitorDetails.availableRect.left,
      ...
      this.desktop.System.getMonitorInfo((monitorInfo) => {
        resolve(
          [monitorInfo.primaryMonitor]
            .concat(monitorInfo.nonPrimaryMonitors)
